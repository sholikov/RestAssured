...
public class HamcrestMatcherWithApiTest {
    ...
    public void helloWorldApiTest() {
        given().accept(ContentType.JSON).when().get("https://sandbox.api.service.nhs.uk/hello-world/hello/world")
                .then().assertThat().statusCode(200).and().contentType("application/json")
                .and().body("message", equalTo("Hello World!"));
    }

    @Test
    public void resResAllUsersTest() {
        ...
        given().accept(ContentType.JSON)
                .when().get("https://reqres.in/api/users")
                .then().assertThat().body("data.id",hasSize(6),//  to work with list
